...
target="${ADOPS_PERRENGUE_CONTAINER_WP_PATH%/}/wp-content/plugins/adrotate/adrotate-adops.php"
command="tmp_plugin=\$(mktemp /tmp/adrotate-adops.XXXXXX.php); printf %s '${payload_base64}' | base64 -d > \"\$tmp_plugin\"; php -l \"\$tmp_plugin\" >/dev/null; mkdir -p \"\$(dirname '${target}')\"; if [ -f '${target}' ]; then cp '${target}' '${target}.bak-'\$(date +%Y%m%d-%H%M%S); fi; install -m 0644 \"\$tmp_plugin\" '${target}'; rm -f \"\$tmp_plugin\"; php -l '${target}'; sha256sum '${target}' | cut -d' ' -f1"
