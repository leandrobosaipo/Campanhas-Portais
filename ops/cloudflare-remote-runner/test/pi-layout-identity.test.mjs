...
function fixture(pi, vehicle) {
  const plain = [
    ...
    `VEÍCULO ${vehicle}`, "MEGA BANNER TOPO - 825 X 120",
  ].join("\n");
  const layout = ["PEDIDO DE                                      PI", `INSERÇÃO     0${pi}`, "VEÍCULO " + vehicle, "01/09 - 30/09"].join("\n");
  ...
  globalThis.fetch = async (url) => {
    ...
    const payload = pathname.endsWith("/clients") ? [{ id: 229, nome: "SANEAR RONDONOPOLIS" }]
      : pathname.endsWith("/agencies") ? [{ id: 79, nome: "IMAGINE" }]
        : pathname.endsWith("/sites") ? [{ id: 22, sigla: "ROO", nome: "ROO Noticias" }, { id: 23, sigla: "AFL", nome: "A Folha Livre" }]
          : [];
...
for (const [name, pi, vehicle] of [
  ["PI 3218 ROO", "03218", "SIITE ROO NOTICIAS"],
  ["PI 3219 AFL", "03219", "SITE A FOLHA LIVRE CUIABÁ"],
]) {
  test(`${name}: full parser accepts layout PI identity and September period`, async (t) => {
