...
const monthlySourceBuilder = await readFile(new URL("../../artifacts/api-server/src/lib/campaign-evidence-export.ts", import.meta.url), "utf8");
...
assert.match(operationsSource, /canonicalSelection/);
assert.match(operationsSource, /compatibleInsertionIds/);
assert.match(monthlySourceBuilder, /canonicalSelection: item\.canonicalSelection/);
assert.match(routeSource, /rotationMode/);
assert.match(routeSource, /capturePolicy: "Aprovar somente evidência/);
assert.match(
  routeSource,
