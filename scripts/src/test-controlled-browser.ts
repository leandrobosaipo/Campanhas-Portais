    ...
    (async () => {
      ...
      try {
        browser = await chromium.launch({
          headless: true,
          ...(process.env.COD5_CHROME_EXECUTABLE_PATH
            ? { executablePath: process.env.COD5_CHROME_EXECUTABLE_PATH }
            : {}),
          args: ["--cod5-browser-test=" + process.env.COD5_BROWSER_MARKER],
        });
  ...
  try {
    await runControlledProcess(process.execPath, ["-e", browserProgram(mode)], {
      ...
      env: {
        ...
        COD5_CHROME_EXECUTABLE_PATH: process.env["ADOPS_CHROMIUM_EXECUTABLE_PATH"]
