...
assert(deploy.includes("perform_verified_rollback || true") && deploy.includes('trap cleanup EXIT'), "deploy precisa de rollback automático no encerramento incompleto");
assert(deploy.includes('bash "$SCRIPT_DIR/deploy-stack.sh" "$ROLLBACK_ENV"'), "rollback precisa reaplicar ambiente anterior");
